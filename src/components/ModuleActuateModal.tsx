/**
 * ModuleActuateModal — right-click context modal for bulk output control.
 *
 * Select a module on the canvas, right-click, and this modal lets you:
 *   - Turn ALL available outputs ON or OFF with a single click.
 *   - For valve body modules, pick which mounted valve slots to actuate.
 *   - See per-channel status feedback.
 */
...
export default function ModuleActuateModal({ open, module: mod, ip, mountedValves, onClose }: Props) {
    ...
    // null = "all selected" (initial default).  Set = explicit selection.
    // Works the same for valve modules and regular I/O modules.
    ...
    async function setOutputs(value: boolean) {
        ...
        // ON: use selected channels.  OFF: if nothing selected, force all channels
        // as a safety measure (previous actuation may have left outputs HIGH).
        const indices = value
            ? [...activeChannels]
            ...
            <DialogContent dividers>
                ...
                {statusMsg && (
                    <Alert
                        ...
                        sx={{ mt: 1.5, py: 0, fontSize: '0.7rem' }}
                    >
                        {statusMsg.text}
                    </Alert>
                )}
